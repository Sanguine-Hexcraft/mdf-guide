import argparse
from mdf.data import load_bands, matches_filter, DATA_PATH


RESET = "\x1b[0m"
BLACK = "\x1b[40m"
WHITE = "\x1b[37m"
GRAY = "\x1b[90m"
CYAN = "\x1b[36m"
BOLD = "\x1b[1m"


def parse_args():
    parser = argparse.ArgumentParser(
        prog="mdf tui",
        description="MDF 2026 Terminal Guide - Black Metal Edition",
        epilog='Examples:\n  mdf tui soundstage\n  mdf tui --stage soundstage --day "Thursday"\n  mdf tui doom --genre death',
    )
    parser.add_argument("search", nargs="*", help="Search terms to filter bands")
    parser.add_argument("--day", help="Filter by day (partial match)")
    parser.add_argument("--stage", help="Filter by stage (partial match)")
    parser.add_argument("--genre", help="Filter by genre (partial match)")
    return parser.parse_args()


def format_rating(value: str, width: int = 4) -> str:
    try:
        rating = int(value)
    except (ValueError, TypeError):
        return " " * width
    if rating == 0:
        s = f"{CYAN}⛧⛧{RESET}"
    elif rating == 1:
        s = f"{CYAN}⛧{RESET} "
    elif rating == 2:
        s = f"{GRAY}✧{RESET} "
    elif rating >= 3:
        s = f"{GRAY}·{RESET} "
    else:
        return " " * width
    return s.ljust(width)


def format_link(url: str) -> str:
    if url:
        return f"{CYAN}⛧{RESET}"
    return ""


def format_row(band: dict, widths: dict) -> str:
    name = band.get("band", "")[: widths["band"]]
    day = band.get("day", "")[: widths["day"]]
    stage = band.get("stage", "")[: widths["stage"]]
    time = band.get("time", "")[: widths["time"]]
    genre = band.get("genre", "")[: widths["genre"]]
    location = band.get("location", "")[: widths["location"]]
    rating = format_rating(band.get("must_see", ""), widths["rating"])
    link = format_link(band.get("metal_archives", ""))

    return (
        f"{WHITE}{name:<{widths['band']}}{RESET} │ "
        f"{GRAY}{day:<{widths['day']}}{RESET} │ "
        f"{stage:<{widths['stage']}} │ "
        f"{CYAN}{time:<{widths['time']}}{RESET} │ "
        f"{genre:<{widths['genre']}} │ "
        f"{location:<{widths['location']}} │ "
        f"{rating} │ "
        f"{link}{RESET}"
    )


def main() -> None:
    args = parse_args()

    if not DATA_PATH.exists():
        print(f"{BLACK}{WHITE}Error: Data file not found{RESET}")
        print(f"  Expected: {DATA_PATH}")
        return

    bands = load_bands(DATA_PATH)
    filtered = [
        b
        for b in bands
        if matches_filter(
            b, search=args.search, day=args.day, stage=args.stage, genre=args.genre
        )
    ]

    if not filtered:
        print(f"{BLACK}{WHITE}No results found{RESET}")
        return

    widths = {
        "band": max(len(b.get("band", "")) for b in filtered) + 1,
        "day": max(len(b.get("day", "")) for b in filtered) + 1,
        "stage": max(len(b.get("stage", "")) for b in filtered) + 1,
        "time": max(len(b.get("time", "")) for b in filtered) + 1,
        "genre": max(len(b.get("genre", "")) for b in filtered) + 1,
        "location": max(len(b.get("location", "")) for b in filtered) + 1,
        "rating": 4,
        "link": 1,
    }

    caps = {"day": 26, "rating": 4, "link": 1}
    widths = {k: min(v, caps.get(k, 20)) for k, v in widths.items()}
    widths["rating"] = 4
    widths["link"] = 1

    filter_desc = "All Bands"
    if args.stage:
        filter_desc = args.stage
    elif args.day:
        filter_desc = args.day

    total_width = (
        widths["band"]
        + widths["day"]
        + widths["stage"]
        + widths["time"]
        + widths["genre"]
        + widths["location"]
        + widths["rating"]
        + widths["link"]
        + 20
    )

    print(f"{BLACK}")
    print(f"╭{'─' * total_width}╮")
    print(
        f"│  {CYAN}☠{RESET} {BOLD}MDF 2026 - {filter_desc}{RESET}{' ' * (total_width - 16 - len(filter_desc))} │"
    )
    print(f"├{'─' * total_width}┤")
    print(
        f"│  {WHITE}Band{' ' * (widths['band'] - 3)}│ Day{' ' * (widths['day'] - 3)}│ Stage{' ' * (widths['stage'] - 5)}│ Time{' ' * (widths['time'] - 4)}│ Genre{' ' * (widths['genre'] - 5)}│ Location{' ' * (widths['location'] - 7)}│ ⚝{' ' * (widths['rating'] - 1)}│ {' ' * widths['link']}{RESET}│"
    )
    print(f"├{'─' * total_width}┤")

    for band in filtered:
        print(f"│ {format_row(band, widths)} │")
        notes = band.get("notes", "")
        if notes:
            print(f"│   {GRAY}→ {notes[: total_width - 5]}{RESET} │")

    print(f"╰{'─' * total_width}╯")
    print(f"{RESET}", end="")


if __name__ == "__main__":
    main()
